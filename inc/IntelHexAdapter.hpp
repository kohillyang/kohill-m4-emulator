...
class Intel_HEX_Adapter: public Object {
	class HEX_LINE {
	public:
		uint8_t startcode;
		int byte_count;
		...
		QString toString() {
			return QString(
					"%1 byte_count:%2 address:0x%L3 record_type:%4 data:0x%5 checksum:%6").arg(
					(char) startcode).arg(byte_count).arg(address, 0, 16).arg(
					record_type).arg(QString(data), 0, 16).arg(checksum);
		}
	};
public:
